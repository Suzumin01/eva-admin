...
import { getDoctors, getSchedules, bulkCreateSchedule, createSchedule, deleteSchedule } from '@/api/endpoints'
...
export function ScheduleAdminPage() {
  ...
  const { data: doctorsData } = useQuery({ queryKey: ['doctors'], queryFn: () => getDoctors({ limit: 100 }) })

  const doctorOptions = [
    { value: '', label: 'Выберите врача' },
    ...(doctorsData?.doctors ?? []).map((d: { doctorId: number; fullName: string }) => ({ value: String(d.doctorId), label: d.fullName })),
  ]

  const { data: slots, isLoading } = useQuery({
    queryKey: ['admin-schedules', doctorId, filterFrom, filterTo],
    enabled: !!doctorId,
    queryFn: () => getSchedules({ doctorId: doctorId!, dateFrom: filterFrom || undefined, dateTo: filterTo || undefined }),
  })

  const bulkMutation = useMutation({
    mutationFn: () => bulkCreateSchedule({ doctorId: doctorId!, dateFrom: bulkFrom, dateTo: bulkTo, times: bulkTimes.split(',').map(t => t.trim()) }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['admin-schedules'] }),
  })

  const singleMutation = useMutation({
    mutationFn: () => createSchedule({ doctorId: doctorId!, slotDate: singleDate, slotTime: singleTime }),
    onSuccess: () => qc.invalidateQueries({ queryKey: ['admin-schedules'] }),
  })
  ...
  return (
    <div className="space-y-5">
      <h1 className="text-xl font-semibold text-gray-900">Расписание врачей</h1>

      <Card>
        <CardHeader><CardTitle>Фильтр</CardTitle></CardHeader>
        <CardContent className="flex flex-wrap gap-3">
          <Select options={doctorOptions} value={String(doctorId ?? '')} onChange={e => setDoctorId(e.target.value ? Number(e.target.value) : null)} className="w-56" />
          <Input type="date" value={filterFrom} onChange={e => setFilterFrom(e.target.value)} className="w-40" placeholder="С" />
          <Input type="date" value={filterTo}   onChange={e => setFilterTo(e.target.value)}   className="w-40" placeholder="По" />
        </CardContent>
      </Card>
      ...
      {doctorId && (
        <Card>
          ...
          <CardContent className="space-y-4">
            <div>
              <p className="mb-2 text-sm font-medium text-gray-700">Массово (диапазон дат)</p>
              <div className="flex flex-wrap gap-3 items-end">
                ...
                <div className="flex-1 min-w-48">
                  ...
                  <Input value={bulkTimes} onChange={e => setBulkTimes(e.target.value)} placeholder="09:00,10:00,..." />
                </div>
                <Button disabled={!bulkFrom || !bulkTo || bulkMutation.isPending} onClick={() => bulkMutation.mutate()}>
                  {bulkMutation.isPending ? 'Создаю...' : 'Создать'}
                </Button>
              </div>
              {bulkMutation.isSuccess && <p className="mt-1 text-xs text-green-600">Готово</p>}
            </div>
      ...
      {doctorId && (
        <Card>
          <CardHeader><CardTitle>Слоты {isLoading ? '' : `(${slots?.length ?? 0})`}</CardTitle></CardHeader>
